...
from app.database import users, get_user_by_username
...
def get_task_group(user, group_name):
    return next(
        (g for g in user["todo"] if g == group_name),
        None
    )


async def update_task_group(user, group_id, group_data):
    update_data = {
...
async def update_task(user, group_id, task_id, task_data):
    ...
    update_data[f"todo.task_groups.$[group].tasks.$[task].updated_at"] = datetime.now()
